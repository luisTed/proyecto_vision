...
<form action="{{route('venta.store')}}" id="formularioVenta" method="POST">
    @csrf
<div class="form-group" style="display:none;">
    <label for="iddelcliente"></label>
    <input type="text" class="form-control" id="iddelcliente" name="iddelcliente" value="0">
</div>
<div class="form-group" style="display:none;">
    ...
    @if($cliente !== null)
<div class="recuadro-blanco" id="iddelcliente">
    <h4>Cliente</h4>
    <p>Nombre: {{ $cliente->nombre }}</p>
</div>
<script>
        setTimeout(function() {
                $("#iddelcliente").val("{{ $cliente->ID_cliente }}");
...
<div class="modal fade" id="cancelarCompraModal" tabindex="-1" aria-labelledby="cancelarCompraModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content bg-danger text-white">
      <div class="modal-header">
        <h5 class="modal-title" id="cancelarCompraModalLabel">Cancelar compra</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body bg-white">
      ...
      <div class="modal-footer bg-white">
        ...
        <button type="button" class="btn btn-success text-white" onclick="confirmarYRedirigir()">Confirmar</button>
    </div>
    ...
    <div class="col-12 mt-4 text-center">
        <a href="#" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#cancelarCompraModal">
            <i class="bi bi-cart-x-fill"></i> Cancelar compra
        </a>

        <button type="button" class="btn btn-success" id="guardar">
            <i class="bi bi-currency-dollar"></i> Realizar compra
        </button>
    </div>
...
<script>
    ...
    // Cerrar el modal
    $('#cancelarCompraModal').modal('hide');
    // Redirigir a la ruta deseada
    window.location.href = "{{ route('bucarclienteexistencia') }}";
}


    //Variables de armazon
    let cont = 0;
    let subtotal = [];
    let sumas = 0;
    let igv = 0;
    ...
    function confirmarVenta(){
        ...
        //   if (result.isConfirmed) {
           document.getElementById('formularioVenta').submit();//------------------------------------asegurar compra
        //   }
        // });
    ...
    function disableButtons(){
        ...
        // Desactivar o activar botón "Cancelar compra" basado en el valor de totalI
        if (totalI == 0) {
            $('#cancelarI').hide();
        } else {
            $('#cancelarI').show();
        ...
        // Si ambos total y totalI son cero, desactivar el botón "Guardar"
    }
                ...
                if (parseFloat(cantidad_maxima) >= parseFloat(cantidad)) {
                    ...
                    let fila = 
                                '<tr id="fila' + cont + '">' +
                                    ...
                                    '<td style="display: none;"><input type="hidden" name="arraycantidadVendidoproducto[]" value="'+cantidad + '">' +"" + '</td>' +
                                '</tr>';
    ...
    function agregarProductoI() {
        ...
        if (nameProductoI != '' && nameProductoI != undefined && cantidadI != '' && precioI != '') {
                ...
                if (parseFloat(cantidad_maximaI) >= parseFloat(cantidadI)) {
                    ...
                    let filaI = 
                                '<tr id="filaI' + contI + '">' +
                                    ...
                                    '<td style="display: none;"><input type="hidden" name="arraycantidadVendidoproductoInsumo[]" value="'+cantidadI + '">' +"" + '</td>' +
                                '</tr>';
            ...
            showModalI('Le faltan campos por llenar o cambie de insumo');
        }
    ...
    function eliminarProductoI(indiceI) {
        //Calcular valores
        sumasI -= roundI(subtotalI[indiceI]);
        // igv2 = roundI(sumasI / 100 * impuesto);
